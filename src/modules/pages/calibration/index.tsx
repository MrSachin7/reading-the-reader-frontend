...
export default function CalibrationPage() {
  ...
  const handleReset = React.useCallback(async () => {
    ...
    dispatch(setStepThreeLastAppliedAtUnixMs(null))
  ...
  return (
    <main className="relative h-screen w-screen overflow-hidden bg-[#f7f2e8] text-slate-950">
      ...
      {phase === "running" && activeTarget ? (
        <div className="pointer-events-none absolute top-28 left-1/2 z-20 -translate-x-1/2 rounded-full border border-slate-900/10 bg-white/88 px-5 py-2 text-sm text-slate-700 shadow-sm">
          {targetPhase === "burst"
            ? `Locked ${activeTarget.label.toLowerCase()}`
            : `Look at ${activeTarget.label.toLowerCase()}`}
        </div>
      ) : null}
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-[1.25rem] border bg-slate-50 p-4">
              ...
              <p className="mt-2 text-lg font-semibold">
                {snapshot?.result?.calibrationPointCount ?? snapshot?.points.length ?? 0}
              </p>
            </div>
            <div className="rounded-[1.25rem] border bg-slate-50 p-4">
